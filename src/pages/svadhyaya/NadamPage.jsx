...
function NadamBg({ isDark }) {
  ...
  return (
    <svg width="420" height="280" viewBox="0 0 420 280" fill="none"
      ...
      <circle cx="340" cy="100" r="60" stroke={COLOR} strokeWidth="1" fill="none" />
      <circle cx="340" cy="100" r="40" stroke={COLOR} strokeWidth="0.75" fill="none" />
      <circle cx="340" cy="100" r="20" stroke={COLOR} strokeWidth="0.75" fill="none" />
      <circle cx="340" cy="100" r="5" fill={COLOR} opacity="0.5" />
      {[0, 45, 90, 135, 180, 225, 270, 315].map((deg, i) => {
        const r = (Math.PI * deg) / 180;
        return <line key={i} x1={340 + 22 * Math.cos(r)} y1={100 + 22 * Math.sin(r)}
          x2={340 + 58 * Math.cos(r)} y2={100 + 58 * Math.sin(r)} stroke={COLOR} strokeWidth="0.5" />;
      })}
    </svg>
  );
}
...
export default function NadamPage() {
  ...
  const safeColor = isDark ? "#DDA74F" : COLOR;
  ...
  return (
    <Box sx={{ minHeight: "100vh", position: "relative", overflow: "hidden" }}>
      ...
      <Box sx={{ p: { xs: 2, md: 3 } }}>
        ...
        {tab === 0 && (
          <>
            <AreaBanner color={safeColor} emoji="🎵" title="Nādam" subtitle={subtitle}
