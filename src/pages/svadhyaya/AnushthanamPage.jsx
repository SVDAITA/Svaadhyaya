...
function AnushthanamBg({ isDark }) {
  ...
  const cx = 350, cy = 150;
  const spokes = [0,30,60,90,120,150,180,210,240,270,300,330].map((deg,i) => {
    const r = (Math.PI * deg) / 180;
    return <line key={i} x1={cx+22*Math.cos(r)} y1={cy+22*Math.sin(r)} x2={cx+75*Math.cos(r)} y2={cy+75*Math.sin(r)} stroke={COLOR} strokeWidth="0.5" opacity="0.8" />;
  });
  const dots = [0,45,90,135,180,225,270,315].map((deg,i) => {
    const r = (Math.PI * deg) / 180;
    return <circle key={i} cx={cx+62*Math.cos(r)} cy={cy+62*Math.sin(r)} r="3" fill={COLOR} opacity="0.5" />;
  });
  return (
    <svg width="420" height="300" viewBox="0 0 420 300" fill="none"
      ...
      <circle cx={cx} cy={cy} r="88" stroke={COLOR} strokeWidth="0.75" fill="none" />
      <circle cx={cx} cy={cy} r="70" stroke={COLOR} strokeWidth="0.75" fill="none" />
      <circle cx={cx} cy={cy} r="50" stroke={COLOR} strokeWidth="0.75" fill="none" />
      <circle cx={cx} cy={cy} r="30" stroke={COLOR} strokeWidth="1" fill="none" />
      <circle cx={cx} cy={cy} r="10" stroke={COLOR} strokeWidth="1" fill="none" />
      <circle cx={cx} cy={cy} r="4" fill={COLOR} opacity="0.6" />
      {spokes}{dots}
      <path d={`M${cx} ${cy-90} L${cx+90} ${cy} L${cx} ${cy+90} L${cx-90} ${cy} Z`} stroke={COLOR} strokeWidth="0.6" fill="none" />
      <path d={`M${cx} ${cy-65} L${cx+65} ${cy} L${cx} ${cy+65} L${cx-65} ${cy} Z`} stroke={COLOR} strokeWidth="0.5" fill="none" />
      <path d="M60 260 C60 230 80 220 70 200 C85 215 90 235 80 260 Z" stroke={COLOR} strokeWidth="0.75" fill="none" />
      <ellipse cx="70" cy="265" rx="14" ry="5" stroke={COLOR} strokeWidth="0.75" fill="none" />
    </svg>
  );
}
